<x-agencia-layout title="Suscripción">
    ...
    @if($suscripcion)
        <div class="mb-6 bg-green-500/10 border border-green-500/20 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center gap-3">
            <div class="flex-1">
                <p class="text-sm font-semibold text-green-400">Plan activo: {{ $suscripcion->plan?->nombre }}</p>
    ...
    @else
        <div class="mb-6 bg-yellow-500/10 border border-yellow-500/20 rounded-2xl p-5">
            <p class="text-sm font-semibold text-yellow-400">Sin suscripción activa</p>
                ...
                @foreach($agencia->suscripciones->take(6) as $sus)
                    <div class="px-5 py-3 flex items-center gap-4">
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-base">{{ $sus->plan?->nombre }}</p>
    ...
    @enderror
    @if(session('error'))
        <div class="mt-4 bg-red-500/10 border border-red-500/20 text-red-400 text-sm rounded-xl px-4 py-3">
            {{ session('error') }}
        </div>
    @endif
