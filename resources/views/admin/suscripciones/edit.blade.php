...
<div class="max-w-md">
    <form method="POST" action="{{ route('admin.suscripciones.update', $sus) }}"
        ...
        <div>
            <label class="block text-sm font-semibold text-base mb-1.5">Fecha de vencimiento *</label>
            <input type="date" name="fecha_vencimiento"
                   value="{{ old('fecha_vencimiento', $sus->fecha_vencimiento->format('Y-m-d')) }}" required
                   class="w-full bg-card2 border border-base text-base rounded-xl px-3 py-2.5 focus:outline-none focus:border-brand-orange transition-colors @error('fecha_vencimiento') border-red-500/60 @enderror">
