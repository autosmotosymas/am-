        ...
        <aside class="fixed inset-y-0 left-0 z-40 w-60 bg-card border-r border-base flex flex-col
            ...
            <div class="border-t border-base p-3 shrink-0 space-y-1">
                <a href="{{ route('home') }}" target="_blank"
                   class="flex items-center gap-2 px-3 py-2 text-xs text-muted hover:text-base hover:bg-card2 rounded-xl transition-colors">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button class="w-full flex items-center gap-2 px-3 py-2 text-xs text-red-400 hover:bg-red-500/5 rounded-xl transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            <header class="h-14 bg-card border-b border-base flex items-center justify-between px-4 md:px-6 shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <h1 class="text-sm font-semibold text-base">{{ $title ?? 'Panel Admin' }}</h1>
                </div>
